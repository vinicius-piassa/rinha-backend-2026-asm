...
A complete io_uring path (multishot accept + multishot recv + provided buffer ring + registered files, including the buf-ring `BUF_LEN` offset bug we hit and fixed) is compiled into the source but `--gc-sections` drops it at link time — Docker's default seccomp profile rejects `io_uring_setup` with `-EPERM` and the rules forbid `security_opt` overrides, so we fall back to the epoll path that does run inside the container.
