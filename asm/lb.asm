...
.cb_done:
    ...
    ; The Nível 3 io_uring path (lb_loop_uring + multishot accept +
    ; registered files + msgpool) is still compiled into the .asm but
    ; --gc-sections drops it because nothing references it from _start:
    ; Docker's default seccomp blocks io_uring_setup, and Rinha 2026
    ; forbids security_opt overrides, so the path can't be reached in
    ; the official bench.  We use the epoll loop here.
    ...
    ; Edge-triggered accept: one CQE per readable-edge.  Our accept_loop
    ; already drains accept4() until -EAGAIN so it's already ET-compatible;
    ; the flag just cuts redundant wakeups under load.
    sub     rsp, 16
    mov     dword [rsp + 0], EPOLLIN | EPOLLET | EPOLLERR
    mov     eax, [tcp_listen_fd]
    mov     qword [rsp + 4], 0
    mov     dword [rsp + 4], eax
    mov     edi, [epoll_fd]
    mov     esi, EPOLL_CTL_ADD
    mov     edx, [tcp_listen_fd]
    lea     r10, [rsp + 0]
