...
.have_paths:
    ...
    ; The full io_uring path (uring_init + register_files + init_buf_ring +
    ; server_loop_uring) stays compiled in the .asm but is dead-code at
    ; link time via --gc-sections.  Docker's default seccomp profile
    ; rejects io_uring_setup with -EPERM, and Rinha 2026 forbids
    ; security_opt overrides, so io_uring can never run in the official
    ; bench.  We keep the epoll path that does run there.
    mov     edi, EPOLL_CLOEXEC
    syscall0 SYS_epoll_create1
    test    eax, eax
    js      .fail_bind
